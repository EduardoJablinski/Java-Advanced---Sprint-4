...
public class JdbcConsumoRepository implements ConsumoRepository {
    ...
    @Override
    public Float getSomaConsumoPorEquipamentoId(long idEquipamento) {
        try {
            return jdbcTemplate.queryForObject(
                    "SELECT COALESCE(SUM(QUANTIDADE_CONSUMO), 0) FROM TB_IASI_CONSUMO WHERE ID_EQUIPAMENTO = ?",
                    new Object[]{idEquipamento},
                    Float.class
            );
        } catch (EmptyResultDataAccessException e) {
            return 0f;
